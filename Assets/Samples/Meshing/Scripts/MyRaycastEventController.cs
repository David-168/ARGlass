...
namespace UnityEngine.XR.ARFoundation.Samples
{
    ...
    /// </summary>
    public class MyRaycastEventController : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            {
                Debug.LogWarning($"{nameof(MyRaycastEventController)} component on {name} has null inputs and will have no effect in this scene.", this);
                return;
            }
        ...
        void ScreenTapped(InputAction.CallbackContext context)
        {
            ...
            GameObject obMainCamera = GameObject.FindWithTag("FaceMesh");

            EyeLocator comEyeLocator = obMainCamera.GetComponent < EyeLocator >();
            var tapPosition = pointer.position.ReadValue();
            Ray tap = new Ray();
            comEyeLocator.getRay(ref tap, tapPosition);

            if (m_ARRaycastHitEvent != null &&
                m_RaycastManager.Raycast(tap, s_Hits, m_TrackableType))
            {
